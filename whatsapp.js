...
const createSession = async (sessionId, isLegacy = false, res = null) => {
    ...
    wa.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            console.log('conected', sessionId)
            try {
                //const response = await axios.get(`http://34.30.68.145:8000/sessions/status/${sessionId}`);
                const whatsapp_object = response.data
                //console.log(whatsapp_object);
                const postData = {
                    device_id: sessionId,
                    whatsapp_number: whatsapp_object,
                    status: 'connected',
                    datetime: Math.floor(Date.now() / 1000),
                }

                // const dataSend = await axios.post('https://test-waportal.kiz.app/api/payload', postData);
                console.log('posted', dataSend)
                console.log('Thanks')
            } catch (error) {
